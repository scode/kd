...
same thing here. "Rehearsal" means a `bootstrap` run with `--target`; "real run" means one without. "The user" on a
rehearsal is the `--target` user; on a real run it is the profile's `user`.
...
- Hostname from the profile; timezone `America/Los_Angeles`.
...
- Hermes: official installer; stop the gateway first if one is running (a real-run rerun can find one); then
  ...
  which a rehearsal must not do: on a real run `--start-now --start-on-login`, on a rehearsal
  `--no-start-now --no-start-on-login`.
- Dashboard: a user systemd unit at `~/.config/systemd/user/hermes-dashboard.service` that runs
...
- Tailscale, real run only: install with the official script if `tailscale` is missing. Enrollment itself stays with kd
  (see the sequence), because it needs the login URL relayed to the terminal.
...
`backup` and `resume` talk to the devbox with plain `ssh` and the user's own config and `known_hosts`; nothing special
is needed because the box hasn't changed. Only `bootstrap` has the problem that the same address comes back with a new
host key, so only bootstrap on a real run uses `-o UserKnownHostsFile=<per-run temp file> -o StrictHostKeyChecking=yes`.
That file is created in the controller's temp directory for the run, filled from the `ssh-keyscan` line the user
confirmed, and discarded afterwards. The user's `~/.ssh/known_hosts` is never touched, which is why `SPEC.md` has the
`ssh-keygen -R` reminder. Every ssh invocation passes `-o ServerAliveInterval=30`.
...
The controller preflight resolves the four agent auth sources without printing them, and fails before anything is
stopped or written if one is missing:
...
target fails, on real runs and rehearsals alike, and the agent consumes it with
...
- `~/.config/systemd/user/hermes-dashboard.service`: the dashboard unit bootstrap writes. `backup` and `resume` use
  `hermes gateway stop` / `hermes gateway start` for the gateway and `hermes dashboard --stop` to stop the dashboard,
  because the devbox being backed up may have a hand-written unit from before kd existed. Starting the dashboard is the
  one place a unit name appears: `systemctl --user start hermes-dashboard` when that unit exists, else
...
1. Controller preflight, cheap and with no connections: the profile parses; `public_key` is readable; the four auth
   sources resolve; `backup_dir` holds at least one `hermes-<hostname>-*.zip`; on a rehearsal, `gh auth token` succeeds.
2. Real run only, every time: `ssh-keyscan -t ed25519 <host>`, show the key's fingerprint in both SHA256 and MD5 forms
   (`ssh-keygen -lf` and `ssh-keygen -E md5 -lf`, consoles vary), ask the user to confirm it matches the console. On
   yes, write the keyscan line to the per-run known-hosts file; on no, exit.
3. Connect as `user@host`; on a real run, if that fails, `root@host` for seeding (a rehearsal has no root and its user
   must already exist). Guard: `pgrep -f '[h]ermes.*gateway'` over that first connection (any user; `hermes` itself may
   not be on PATH yet). If it matches, a real run asks "Hermes gateway is running on the target; continue anyway?" and a
   rehearsal refuses outright. Running, not enabled, is the test, and directories are never checked, because a rerun
   after a partial bootstrap must work: on a real run the previous attempt may already have restored Hermes, and on a
   rehearsal Hermes is installed without being started.
...
profile hostname; `timedatectl show -p Timezone --value` equals `America/Los_Angeles`; `gh auth status`; the count of
`~/git/*` directories equals the size of the manifest deduplicated with `scode/voice` and `scode/dotfiles`;
`ssh -o BatchMode=yes localhost true`; `hermes gateway status`, whose exit status is 0 either way, so its own output
line is printed with no verdict; `curl -fsS 127.0.0.1:9119/api/status` (skipped on a rehearsal); `docker ps`;
`tailscale status` (skipped on a rehearsal); and one real request per agent CLI:
`codex exec --skip-git-repo-check "reply ok"`, `claude -p ok`, `opencode run ok`, `muse exec ok`. Every check is
reported; none is fatal.

### Backup sequence

Controller preflight (profile parses, four auth sources resolve, `backup_dir` created if missing); read-only preflight
script on the devbox: `pgrep -af '[c]odex|[c]laude|[o]pencode|[m]use|[h]ermes'`, `tmux ls`, and the per-repo dirty and
unpushed check below; prompt unless `--yes`; `hermes gateway stop` and `hermes dashboard --stop`, errors ignored if
nothing was running (skipped with `--keep-running`); `hermes backup -o ~/hermes-backup-kd.zip`; completeness check: exit
status 0, nonzero file size, and no line containing `incomplete` on stderr (the observed failure on the real box was a
ZIP-valid archive that Hermes itself called incomplete because of live sockets, so the exit status alone is not enough;
with `--keep-running` the socket warning is expected and only the exit status and size are checked); `sha256sum` on the
devbox, pull, compare with the pulled file's hash, `chmod 600`, delete the devbox copy; print the first line of
`hermes --version`; leave Hermes stopped (skipped with `--keep-running`). On backup failure, restart the gateway and
dashboard before exiting (skipped with `--keep-running`, which never stopped them).
...
Very little of this is unit-testable. Unit tests cover profile parsing and transport command lines; the prompts and the
probe script are string templates that only change when someone edits them on purpose, so they get no snapshot tests.
...
kd devbox bootstrap --profile <name> --target scode@<worker>
```

Both are unattended. Rerunning bootstrap against the same worker is the fast loop for iterating on a later phase; a
fresh worker is the full check. Neither touches the real devbox beyond reading it and writing one archive that is
removed again.
